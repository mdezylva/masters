...
pair_index = int(input("Enter a pair index to test: "))

galaxy_1 = cut_pairs_df.loc[pair_index]['galaxy_index_1']
galaxy_2 = cut_pairs_df.loc[pair_index]['galaxy_index_2']

pair = [galaxy_1,galaxy_2]

cut_array = galaxy_pairs.cut_out_pair(pair, y_map_array, cut_df)

plt.imshow(cut_array)
plt.show()

gal_1_coords = galaxy_pairs.extract_ra_dec(galaxy_1, cut_df)
print(gal_1_coords)
gal_2_coords = galaxy_pairs.extract_ra_dec(galaxy_2, cut_df)
print(gal_2_coords)
angle = galaxy_pairs.get_rotn_angle(gal_1_coords, gal_2_coords,debug= False)
print(angle)
